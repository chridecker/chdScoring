...
$id = $res->id + 1;
if($_GET['id'] == "*"){
	$query = "INSERT INTO judge (`id`,`name`,`vorname`,`license`,`pin`,`editscore`) VALUES (".$id.",'".$_GET['name']."','".$_GET['vorname']."','".$_GET['license']."','".$_GET['pin']."','".$_GET['editscore']."')";
...
if($_GET['id'] != "*" && $_GET['id'] != "" && $_GET['id'] != "del"){
    $editScores = $_GET['editscore'];
	$query = "UPDATE judge SET name = '".$_GET['name']."', vorname = '".$_GET['vorname']."', license = '".$_GET['license']."', pin = '".$_GET['pin']."', editscore = ".($editScores)." WHERE id = ".$_GET['id'];
	mysqli_query($link,$query);
...
<table class="liste">
<tr>
<th colspan="6" class="headline">Judges</th>
...
<tr class="header">
<th>#</th><th>Name</th><th>Surname</th><th>License</th><th>Pin</th><th>Edit Score</th><th>[X]</th></tr>
<?php
$count = 1;
while($obj = mysqli_fetch_object($res)){?>
    ...
    <td><input type="checkbox" id="editscore<?php echo $obj->id;?>" <?php if($obj->editscore == 1) echo "checked";?> onChange="speichern(<?php echo $obj->id;?>);" /></td>
...
?>
    ...
    <td><input type="checkbox" id="editscore*" /></td>
