...
function changeControl(type){
	if(type == 'keyboard'){
		...
		setCookie(type,controlKeyboard,1);
	}
	if(type == 'gamepad'){
		controlGamepad = !controlGamepad;
		setCookie(type,controlGamepad,1);
	}
	control();
}
function control(){
...
function process(){
	if(xmlHttp){
		try{
			xmlHttp.open("GET","wettkampf.php",true);
...
function vorflieger(airfield){
	if(xmlHttp2){
		try{
			xmlHttp2.open("GET","operations/vorflieger.php",true);
			xmlHttp2.send(null)
		}
		catch(e){
			alert("Can't connect to Server: " + e.toString());
...
function suche(){
	var svalue = prompt("Open Participant by number # ","");
	if(svalue != "" && svalue != null)oeffnen("teilnehmer_detail.php?id=" + svalue);
}
