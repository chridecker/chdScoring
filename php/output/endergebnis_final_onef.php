...
if(isset($_GET["db"])){$link = mysqli_connect($host,$user,$password,$_GET["db"]); $database=$_GET['db'];}
$file = "../config.xml";
...
if($res_durchgaenge->md <= 1){
	...
	while($teilnehmer = mysqli_fetch_object($res)){	
		...
		mysqli_query($link,$query_result);
		mysqli_query($link,"ALTER TABLE bewerb".$bewerb." ADD declined float");
		$query_gesamt = "SELECT min(prom) as min FROM bewerb".$bewerb." WHERE teilnehmer = ".$teilnehmer->teilnehmer." AND durchgang < ".$final_durchgang.";";
		$res_gesamt = mysqli_fetch_object(mysqli_query($link,$query_gesamt));
		...
		$query_result = "UPDATE bewerb".$bewerb." SET prom = prom - ".$res_gesamt->min." WHERE durchgang = ".($durchgaenge+1)." AND teilnehmer = ".$teilnehmer->teilnehmer;
		mysqli_query($link,$query_result);
	}
}
//if(isset($logo)){}
...
<th>Rank</th><th>Name</th><th><?php /*echo "Club / ";*/?>Nation</th><th>FAI-License</th>
<?php
...
while($teilnehmer = mysqli_fetch_object($result_teilnehmer)){
    ...
    <?php
	$query_min = "SELECT min(prom) as min FROM bewerb".$bewerb." WHERE teilnehmer = ".$teilnehmer->id." AND durchgang < ".$final_durchgang;
