...
while($teiln = mysqli_fetch_object($res_teilnehmer_durchgaenge)){
	if($teiln->anzahl_durchgang >= $end_final_durchgang && $end_finale == 1){
		//End Finale Urkunden
		echo $decline = mysqli_fetch_object(mysqli_query($link,"SELECT min(prom) as decline FROM bewerb".$bewerb." WHERE (durchgang = ".$end_final_durchgang." OR durchgang = ".($end_final_durchgang + 2).") AND teilnehmer = ".$teiln->teilnehmer))->decline;
...
if($result_teilnehmer = mysqli_query($link,$query_teilnehmer)){
	...
	while($teilnehmer = mysqli_fetch_object($result_teilnehmer)){ ?>
		...
		elseif ($end_finale == 0 && $finale == 0) echo number_format(($teilnehmer->gesamt * ($durchgaenge - 1)),2);
        else echo number_format($teilnehmer->gesamt,2);?>
