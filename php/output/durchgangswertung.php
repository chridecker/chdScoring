...
if($_SERVER['PHP_SELF'] == "/output/durchgangswertung.php")require_once("../host.inc");
...
if(isset($durchgang));
	...
	if($max->durchgang == 0)$durchgang = $_GET['airfield'];
		...
		$query_anzahl = "SELECT count(wl.teilnehmer) as anzahl FROM wettkampf_leitung wl WHERE wl.durchgang = ".$durchgang." AND wl.status > 0";
...
$query_durchgang = "SELECT t.*, concat(t.vorname,' ',t.nachname) as name, d.wert_abs as abs, d.wert_prom as prom FROM teilnehmer as t, durchgang as d WHERE t.id  = d.teilnehmer AND d.durchgang = ".$durchgang." ORDER BY d.wert_abs DESC";
